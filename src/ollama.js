...
const SYSTEM_PROMPT = `You are an expert Indian markets (NSE) intraday analyst.
You are given two live screenshots captured at the same moment:
  IMAGE 1 = Zerodha Kite TradingView chart of the NIFTY 50 index, showing today's intraday session
            in 1-minute candles (price action, candles, volume, any indicators visible).
  IMAGE 2 = Sensibull "OI vs Strike" chart for NIFTY options (Call OI and Put OI bars per strike, plus change-in-OI if shown).
...
Be concrete: quote the strike numbers, price levels and OI observations you can actually see.
This is technical analysis for study purposes, not investment advice.`;

const USER_PROMPT = `You have now seen both images, the session memory and the live price feed.
IMAGE 1 = Kite NIFTY 50 price chart. IMAGE 2 = Sensibull NIFTY OI vs Strike chart.
...
- "highest_call_oi_strike"/"highest_put_oi_strike": the strike numbers with the tallest bars in IMAGE 2.
...
const DESCRIPTIONS = {
  kite: 'the Zerodha Kite TradingView chart of the NIFTY 50 index (today\'s intraday session, 1-minute candles)',
...
export async function analyse(shots, trend = null, quote = null, { onToken } = {}) {
  const usable = shots.filter((s) => s.ok && s.base64);
  if (usable.length === 0) {
    return { ok: false, error: 'No screenshots available to analyse.' };
  }

  const blocked = shots.filter((s) => s.awaitingLogin).map((s) => s.label);
  if (blocked.length) {
    return {
      ok: false,
      skipped: true,
...
export default { analyse, checkOllama };
